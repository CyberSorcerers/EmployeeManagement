...
ngOnInit():void {
  ...
  private getAllQualifications() {
    this.dataRequest.getQualifications().subscribe(qualifications => {
      ...
      qualifications.forEach(qualification => {
        skillsGroup.addControl(qualification.id.toString(), new FormControl(false));
      });

      if (!this.isAddMode) {
        this.dataRequest.getEmployeesById(this.id?.toString() || null)
          .subscribe(employee => {
  ...
  onSubmit() {
    ...
    employeeData.skillSet = this.qualifications.filter(qualification => {
      return this.employeeForm.get('skills')?.get(qualification.id.toString())?.value;
  ...
  private createEmployee(employeeData: any) {
    this.dataRequest.createEmployee(employeeData)
      .subscribe(() => {
  ...
  private updateEmployee(employeeData: any) {
    this.dataRequest.updateEmployee(this.id, employeeData)
      .subscribe(() => {
  ...
  deleteSkill(skillId: number) {
    const skillsGroup = this.employeeForm?.get('skills') as FormGroup;
    if (skillsGroup.contains(skillId.toString())) {
      skillsGroup.removeControl(skillId.toString());
    }
  }
}
